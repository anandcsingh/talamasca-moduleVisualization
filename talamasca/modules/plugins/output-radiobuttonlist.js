CORE.create_module("output-radiobuttonlist", function (sb) {
    var me;
    ...
    return {
        init: function () {
            me = this;
            sb.listen({
                'output-radiobuttonlist': me.processData
            });
        },
                                ...
                                for (i = 0; i < data.value.length; i++) {
                                    var content = '<div>' +
                                                    '<span style="display:inline-block">' +
                                                        '<input id="' + elementName + '" name="' + elementName + '" type="radio" value="' + data.value[i] + '">' +
                                                    '</span>' +
                                                    '<span style="display:inline-block">' +
        ...
        destroy: function () {
            sb.ignore(['output-radiobuttonlist']);
            me = null;
        }
    };
});